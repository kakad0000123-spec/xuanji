...
> ✅ 已實作（2026-07-07）：`drawShareCard(cfg)` 整張重畫——深黑棕背景＋程序式金粉紋＋頂部月牙、中央 ivory 紙籤卡（朱紅上下壓條＋金框＋金色徽章）、玄問 墨色字標、cartouche badge、垂直籤詩、金框釋義盒、footer lockup（左 中文日期/農曆 · 中 分隔 · 右 朱紅印章＋玄問＋極淡 domain）。求籤/抽卦/塔羅/合參/今日運勢/配對/**今日氣場**全部改用此卡（今日氣場先前漏接文字分享，一併補上）。踩坑：canvas wrapText 要先設字型再量寬，否則換行寬度算錯會溢出。
