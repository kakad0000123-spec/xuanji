...
## 由來
2026-07-07 由 `~/Downloads/玄機籤閣.html`（原本只有求籤／抽卦／擲筊，頭部干支・節氣為近似值）搬進 `~/code/xuanji` 立為專案，新增農民曆分頁並把頭部干支／節氣改用 lunar.js 精算。同日品牌改名：**玄機籤閣 → 玄問**（「牘」難唸難打、「玄問」直指占問本質，保留「玄」的品牌資產）。
